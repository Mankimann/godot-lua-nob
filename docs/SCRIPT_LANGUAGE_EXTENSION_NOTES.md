...
| `LuaScript` | Implementiert als `ScriptExtension`-Resource mit Source-Code-, Datei-, Methodendaten und nativem `GDExtensionScriptInstanceInfo3`-Pfad. | Nächster UX-Schritt ist die direkte Anzeige im originalen Godot-Script-Resource-Picker. |
...
| `godot_lua_editor` | Editor-Plugin mit `.lua`-Importer, Inspector-nahem Lua-Script-Slot und Menüaktion für ausgewählte Nodes. | Kann später noch in Godots eingebauten Script-Resource-Picker integriert werden. |
...
Godot verlangt für eine vollständige Sprachintegration zahlreiche virtuelle Methoden, Editor-Hooks und einen nativen `GDExtensionScriptInstance`-Descriptor. Der Descriptor ist jetzt als Grundversion vorhanden und delegiert an den bestehenden `LuaScriptInstance`-Runtime-Wrapper. Import, Inspector-naher Lua-Slot und Menü-Attach sind ebenfalls vorhanden. Tiefere Debugger-, Autocomplete-, Original-Script-Picker- und Property-Inspector-Integration bleibt als Ausbaupunkt dokumentiert.
